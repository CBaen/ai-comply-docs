...
export function generateConsumerRightsProcedures(
  ...
  y = addWrappedText(
    doc,
    `These procedures govern ${data.company.name}'s response to consumer rights requests under the New Jersey Data Protection Act (S332/A1971, signed January 16, 2024, effective January 15, 2025). The controller must respond to authenticated consumer requests within 45 days, with the option to extend by an additional 45 days with notice to the consumer. Note: The NJDPA is a separate law from the NJ Law Against Discrimination.`,
    MARGIN,
    y,
    CONTENT_WIDTH,
    LINE_HEIGHT
  ...
  y = addWrappedText(
    doc,
    "Per the NJDPA, the controller must take reasonable steps to authenticate consumer requests before responding. Recommended Best Practice \u2014 not a statutory mandate: do not require information beyond what is reasonably necessary to verify identity.",
    MARGIN,
    y,
    CONTENT_WIDTH,
    LINE_HEIGHT
  ...
  const rightsAndProcedures = [
    ...
    {
      right: "Right to Portability (NJDPA)",
      procedure:
    ...
    {
      right: "Right to Opt Out of Targeted Advertising (NJDPA)",
      procedure:
  ...
  y = addWrappedText(
    doc,
    "Per the NJDPA, if the controller declines to act on a request, the consumer has the right to appeal. The controller must provide an appeals process and inform the consumer of the outcome and reasons for the decision. If the appeal is denied, the controller must provide information about how to contact the New Jersey Attorney General to submit a complaint.",
    MARGIN,
    y,
    CONTENT_WIDTH,
    LINE_HEIGHT
