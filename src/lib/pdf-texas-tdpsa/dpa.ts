...
export function generateDataProtectionAssessment(data: ComplianceFormData): jsPDF {
  ...
  y = addWrappedText(
    doc,
    `This Data Protection Assessment is conducted by ${data.company.name} pursuant to Tex. Bus. & Com. Code \u00A7 541.107(a), which requires controllers to conduct and document a data protection assessment for processing activities that present a heightened risk to consumers. Covered activities include processing for targeted advertising, sale of personal data, profiling for consequential decisions, and processing sensitive data. These assessments must be made available to the Texas Attorney General upon request (\u00A7 541.107(b)).`,
    MARGIN,
    y,
    CONTENT_WIDTH,
    LINE_HEIGHT
  ...
  data.aiSystems.forEach((sys, idx) => {
    ...
    y = addWrappedText(
      doc,
      "Processing activities triggering this assessment (per \u00A7 541.107(a)) \u2014 check all that apply:",
      MARGIN,
      y,
      CONTENT_WIDTH,
      LINE_HEIGHT
    ...
    const triggers = [
      "Processing for targeted advertising (\u00A7 541.107(a)(1))",
      "Sale of personal data (\u00A7 541.107(a)(2))",
      "Processing for profiling \u2014 consequential decisions (\u00A7 541.107(a)(3))",
      "Processing sensitive data (\u00A7 541.107(a)(4))",
    ];
    ...
    y = addWrappedText(
      doc,
      "Risks to consumers \u2014 including potential for discrimination based on protected class (\u00A7 541.107(a)):",
      MARGIN,
      y,
      CONTENT_WIDTH,
      LINE_HEIGHT
  ...
  y = addWrappedText(
    doc,
    "Recommended Best Practice \u2014 not a statutory mandate: review assessments annually and when processing activities change significantly. Retain assessments and make available to the Texas AG upon request (\u00A7 541.107(b)).",
    MARGIN,
    y,
    CONTENT_WIDTH,
    LINE_HEIGHT
