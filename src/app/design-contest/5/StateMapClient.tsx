...
function getUniqueJurisdictions(regulations: Regulation[]) {
  ...
  return regulations.filter((r) => { if (seen.has(r.state)) return false; seen.add(r.state); return true; });
}
...
function getLawNamesForJurisdiction(regs: Regulation[], state: string): string[] {
  return regs.filter((r) => r.state === state).map((r) => r.shortName).slice(0, 2);
}
...
function JurisdictionCard({ state, regulations, isActive, onClick }: { state: string; regulations: Regulation[]; isActive: boolean; onClick: () => void }) {
  ...
  return (
    <button
      ...
      className={`group relative overflow-hidden rounded-xl text-left transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${isActive ? "ring-2 ring-blue-500 shadow-lg shadow-blue-500/20 scale-[1.03]" : "hover:scale-[1.02] hover:shadow-md"}`}
    >
      <div className="relative h-24 sm:h-28">
        {meta.photo ? (
          <>
            <img src={`/images/landing/${meta.photo}`} alt="" aria-hidden="true" className="absolute inset-0 w-full h-full object-cover" />
        ...
        <div className="absolute bottom-0 left-0 right-0 px-3 pb-2">
          <div className="flex items-end justify-between">
            <span className="text-2xl font-extrabold text-white font-display leading-none drop-shadow">{meta.abbr}</span>
            <span className="text-[10px] text-white/70 font-medium leading-none mb-0.5">{productCount} {productCount === 1 ? "package" : "packages"}</span>
          </div>
        </div>
      </div>
      <div className={`bg-white border-x border-b rounded-b-xl px-3 py-2 ${isActive ? "border-blue-400" : "border-gray-200 group-hover:border-blue-200"} transition-colors`}>
        <p className="text-xs font-bold text-gray-900 leading-snug truncate">{state}</p>
        {lawNames.map((name) => <p key={name} className="text-[10px] text-gray-500 truncate leading-tight mt-0.5">{name}</p>)}
      </div>
    </button>
  );
}
...
function CoverageStats({ regulations }: { regulations: Regulation[] }) {
  ...
  return (
    <div className="grid grid-cols-2 sm:grid-cols-4 gap-0 border-t border-b border-slate-700/50 mb-0">
      {stats.map((s, i) => (
        <div key={s.label} className={`py-5 px-4 text-center ${i < stats.length - 1 ? "border-r border-slate-700/40" : ""}`}>
          <div className="text-2xl sm:text-3xl font-extrabold text-white font-display leading-none mb-1">{s.value}</div>
...
function CoverageGrid({ regulations, activeState, onStateClick }: { regulations: Regulation[]; activeState: string | null; onStateClick: (state: string | null) => void }) {
  const uniqueJurisdictions = getUniqueJurisdictions(regulations);
  const byRegion: Record<RegionKey, string[]> = { state: [], federal: [], international: [] };

  uniqueJurisdictions.forEach((r) => {
    const meta = JURISDICTION_META[r.state];
    const region: RegionKey = meta?.region ?? "state";
    if (!byRegion[region].includes(r.state)) byRegion[region].push(r.state);
  });
  ...
  return (
    <div className="space-y-6">
      {(["state", "federal", "international"] as RegionKey[]).map((region) => {
        ...
        return (
          <div key={region}>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-3 px-1">{REGION_LABELS[region]}</h3>
            <div className={`grid gap-3 ${region === "state" ? "grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5" : "grid-cols-2 sm:grid-cols-3 md:grid-cols-4"}`}>
              {states.map((state) => (
                <JurisdictionCard
                  key={state}
                  state={state}
...
function ProductCard({ reg, highlightState }: { reg: Regulation; highlightState: string | null }) {
  ...
  return (
    <div className={`bg-white rounded-lg border border-gray-200 p-4 sm:p-5 flex flex-col border-l-[3px] ${accentColor} transition-all duration-200 ${isHighlighted ? "hover:border-blue-700 hover:shadow-md opacity-100" : "opacity-40 pointer-events-none"}`}>
      <div className="flex justify-between items-start gap-2 mb-1">
        <h3 className="font-bold text-sm sm:text-base font-display text-gray-900 leading-snug">{reg.shortName}</h3>
        <span className={`shrink-0 inline-flex items-center gap-1.5 text-[10px] sm:text-xs px-2 py-0.5 rounded font-bold ${statusStyles[reg.status] ?? statusStyles.proposed}`}>
          <span aria-hidden="true" className={`inline-block w-1.5 h-1.5 rounded-sm ${reg.status === "in-effect" ? "bg-red-500" : reg.status === "effective-soon" ? "bg-amber-500" : "bg-slate-400"}`} />
      ...
      <p className="text-gray-600 text-xs sm:text-sm mb-4 leading-relaxed flex-1">{firstSentence}</p>
      <div className="flex items-end justify-between mb-3 pt-3 border-t border-gray-100">
        <div>
          <span className="text-xl sm:text-2xl font-extrabold text-gray-900 font-display leading-none">${reg.price}</span>
      ...
      {reg.ready ? (
        <Link href={`/products/${reg.slug}`} className="block text-center bg-blue-800 text-white py-2.5 rounded-lg font-semibold text-sm hover:bg-blue-900 transition">See Details</Link>
      ) : (
        <Link href={`/products/${reg.slug}`} className="block text-center bg-slate-50 border border-gray-200 text-gray-600 py-2.5 rounded-lg font-semibold text-sm hover:border-blue-300 hover:text-blue-700 transition">Coming Soon — Learn More</Link>
      )}
    </div>
  );
}
...
function DeadlineBanner({ regulations }: { regulations: Regulation[] }) {
  const urgent = regulations
    .filter((r) => r.status === "effective-soon" || (r.status === "in-effect" && r.effectiveDate.includes("2026")))
    .slice(0, 4);
  ...
  return (
    <div className="bg-red-50 border border-red-200 rounded-xl p-5 mb-8">
      <div className="flex items-center gap-2 mb-1">
        ...
        <h3 className="font-bold text-red-900 text-sm font-display">These Laws Are In Effect Now — Penalties Are Live</h3>
      </div>
      <p className="text-[11px] text-red-600 mb-3">Last updated {new Date().toLocaleDateString("en-US", { month: "long", day: "numeric", year: "numeric" })}</p>
      <div className="grid sm:grid-cols-2 gap-2">
        {urgent.map((r) => {
          const effectiveParsed = new Date(r.effectiveDate);
          const isPast = !isNaN(effectiveParsed.getTime()) && effectiveParsed < new Date();
          return (
            <Link key={r.slug} href={`/products/${r.slug}`} className="flex items-center justify-between bg-white border border-red-100 rounded-lg px-3 py-2.5 hover:border-red-300 transition group gap-2">
              <div className="min-w-0">
                <p className="font-semibold text-gray-900 text-xs group-hover:text-blue-700 transition truncate">{r.shortName}</p>
              ...
              <p className="text-[10px] font-bold text-red-700 shrink-0 text-right leading-snug">
                {isPast ? "In effect since" : "Effective"}<br />{r.effectiveDate}
              </p>
            </Link>
          );
        })}
...
export default function ProductLibraryV2({ regulations }: { regulations: Regulation[] }) {
  ...
  const displayed = activeState ? searchFiltered.filter((r) => r.state === activeState) : searchFiltered;
  ...
  return (
    <>
      ...
      <header className="bg-slate-900 text-white">
        <div className="relative overflow-hidden">
          <div className="absolute inset-0">
            <img src="/images/landing/product-tablet-desk.png" alt="" aria-hidden="true" className="w-full h-full object-cover opacity-20" />
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-12 relative z-10">
            ...
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-extrabold font-display text-white mb-3 leading-tight tracking-tight">AI Compliance Templates</h1>
            <p className="text-slate-300 text-sm sm:text-base max-w-2xl leading-relaxed">State-specific and universal AI compliance packages, each built against the enacted statute text. Select your jurisdiction below or browse all products.</p>
          </div>
        </div>
        <CoverageStats regulations={regulations} />
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8 sm:py-10">
          <div className="flex items-center justify-between mb-5">
            <h2 className="text-sm font-bold text-slate-200 uppercase tracking-widest">Jurisdictions Covered</h2>
            {activeState && (
              <button onClick={() => handleStateClick(null)} className="text-xs text-blue-400 hover:text-blue-300 transition font-medium flex items-center gap-1">
                <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                Clear filter
              </button>
            )}
          </div>
          <CoverageGrid regulations={regulations} activeState={activeState} onStateClick={handleStateClick} />
          {activeState && (
            <p className="mt-5 text-sm text-blue-300 text-center">
              Showing packages for <strong className="text-white">{activeState}</strong> —{" "}
              <button onClick={() => handleStateClick(null)} className="underline hover:no-underline">show all</button>
            </p>
          )}
        </div>
      </header>
      ...
      {/* Product section */}
      <section ref={productSectionRef} className="py-10 sm:py-14 bg-slate-50" aria-label="Product packages">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-7">
            <div className="flex-1">
              <h2 className="text-lg sm:text-xl font-bold font-display text-gray-900">
                {activeState ? `${activeState} Packages` : "All Compliance Packages"}
              </h2>
              <p className="text-xs text-gray-500 mt-0.5">
                {displayed.length} package{displayed.length !== 1 ? "s" : ""} shown
                {activeState && <> · <button onClick={() => setActiveState(null)} className="text-blue-700 hover:underline">clear filter</button></>}
              </p>
            </div>
          ...
          {displayed.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-5">
              {displayed.map((reg) => <ProductCard key={reg.slug} reg={reg} highlightState={activeState} />)}
            </div>
          ) : (
          ...
          <details className="mt-10 group">
            <summary className="cursor-pointer list-none flex items-center gap-2 text-sm font-semibold text-slate-700 hover:text-blue-700 transition select-none">
              <svg className="w-4 h-4 text-blue-600 shrink-0 transition-transform group-open:rotate-90" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2} aria-hidden="true">
            ...
            <div className="mt-3 bg-white border border-slate-200 rounded-xl p-5 grid sm:grid-cols-3 gap-6">
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">Employees in Illinois, NYC, or Colorado + automated hiring tools:</p>
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">Using customer data for targeting, profiling, or automated decisions:</p>
                <ul className="space-y-1">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500 mb-3">Start with an AI inventory before choosing which laws apply:</p>
                <ul className="space-y-1">
