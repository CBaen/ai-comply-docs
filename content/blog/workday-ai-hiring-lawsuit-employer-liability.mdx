---
title: "Workday AI Hiring Lawsuit Could Reshape Employer Liability"
slug: "workday-ai-hiring-lawsuit-employer-liability"
description: "Mobley v. Workday cleared class certification — vendor-liability claims for AI hiring discrimination are proceeding. Class could cover 1.1B+ applications. What every employer using AI in hiring needs documented now."
date: "2026-03-31"
author: "AI Compliance Documents Team"
tags: ["Mobley v. Workday", "AI hiring", "agent liability", "ADEA", "Title VII", "employment discrimination", "AI vendor liability"]
published: true
image: "/blog/blog-hero-workday.png"
summary: "A federal court has allowed discrimination claims against Workday's AI screening tools to proceed on an unprecedented 'agent' liability theory, with conditional certification covering what could be hundreds of millions of job applicants. The case, Mobley v. Workday, Inc., is the first major test of whether AI vendors — not just employers — can be held directly liable for algorithmic hiring bias under federal anti-discrimination law."
cardSummary: "A federal court is testing whether AI vendors — not just employers — can be sued for discriminatory hiring outcomes. The certified class could include hundreds of millions of applicants."
deepDive:
  title: "Agent Liability in Employment Discrimination Law"
  content: "When you apply for a job through a company's website, the software screening your resume might not be built or operated by that company at all. It might be built by a third-party vendor like Workday. So when that software discriminates — rejecting applicants based on race, age, or disability — the question becomes: who is legally responsible?\n\nFederal anti-discrimination laws like [Title VII](https://www.eeoc.gov/laws/guidance/employment-tests-and-selection-procedures), the [ADEA](https://www.eeoc.gov/age-discrimination), and the [ADA](https://www.eeoc.gov/disability-discrimination) define 'employer' broadly to include any 'agent' of the employer. Historically, this language was understood to apply to individual managers or HR staff who carried out discriminatory decisions on behalf of the company. A hiring manager who refused to interview older applicants was an agent of the employer and could create liability for the company.\n\nThe Mobley case asks whether a software vendor can be that agent. The argument is straightforward: when an employer subscribes to Workday's platform and delegates the screening of job applicants to Workday's AI system, Workday is performing a function that the employer has authorized and controls. The employer sets the criteria. The vendor's system applies those criteria (and potentially adds its own biases from training data). The applicant is rejected. The functional relationship looks a lot like an agency relationship.\n\nThe court in Mobley did not definitively rule that Workday is an agent — it ruled that the theory was plausible enough to survive a motion to dismiss. But even this preliminary endorsement has enormous implications. It means that AI vendors cannot simply disclaim responsibility for outcomes on the grounds that they are merely selling a tool. If the tool is performing an employer function, the vendor may be performing an employer function too. The [EEOC's amicus brief](https://www.eeoc.gov/litigation/briefs/mobley-v-workday-inc) explicitly supported this reading of the law, giving it the weight of the federal enforcement agency's formal legal position."
microFacts:
  - fact: "Workday disclosed in court filings that its software tools rejected approximately 1.1 billion job applications during the relevant period beginning September 24, 2020."
    source: "GovInfo — Court Order"
    sourceUrl: "https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf"
  - fact: "NYC's Department of Consumer and Worker Protection received only two complaints related to automated employment decision tools during the entire July 2023 – June 2025 period covered by the State Comptroller's audit of LL144 enforcement."
    source: "NY State Comptroller"
    sourceUrl: "https://www.osc.ny.gov/state-agencies/audits/2025/12/02/enforcement-local-law-144-automated-employment-decision-tools"
  - fact: "The NY State Comptroller found at least 17 instances of potential non-compliance among 32 companies reviewed, while DCWP's own review of the same companies found only one."
    source: "NY State Comptroller"
    sourceUrl: "https://www.osc.ny.gov/state-agencies/audits/2025/12/02/enforcement-local-law-144-automated-employment-decision-tools"
  - fact: "The EEOC's 2023 settlement with iTutorGroup — for programming its software to automatically reject applicants over certain ages — remains the first resolved federal case involving AI-driven hiring discrimination. The company paid $365,000."
    source: "EEOC"
    sourceUrl: "https://www.eeoc.gov/newsroom/itutorgroup-pay-365000-settle-eeoc-discriminatory-hiring-suit"
externalReferences:
  - title: "EEOC Amicus Brief Listing — Mobley v. Workday, Inc."
    url: "https://www.eeoc.gov/litigation/briefs/mobley-v-workday-inc"
  - title: "EEOC Amicus Brief (Full PDF)"
    url: "https://www.eeoc.gov/sites/default/files/2024-04/Mobley%20v%20Workday%20NDCal%20am-brf%2004-24%20sjw.pdf"
  - title: "GovInfo — Court Order (May 16, 2025, conditional certification ruling)"
    url: "https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf"
  - title: "EEOC — iTutorGroup Settlement Press Release"
    url: "https://www.eeoc.gov/newsroom/itutorgroup-pay-365000-settle-eeoc-discriminatory-hiring-suit"
  - title: "NY State Comptroller — LL144 Enforcement Audit (Dec. 2, 2025)"
    url: "https://www.osc.ny.gov/state-agencies/audits/2025/12/02/enforcement-local-law-144-automated-employment-decision-tools"
  - title: "NYC DCWP — Automated Employment Decision Tools (LL144)"
    url: "https://www.nyc.gov/site/dca/about/automated-employment-decision-tools.page"
---

A federal court has ruled that discrimination claims against Workday's AI hiring tools can proceed — not just against the employers who used them, but against Workday itself. The case is *Mobley v. Workday, Inc.*, and it names every employer using an AI screening tool as a potential defendant in the same type of claim. If the court's theory holds, your vendor contract does not protect you.

Here's what you need to understand about this case before your next hiring cycle.

## What is the Mobley v. Workday case and what claims does it make against an AI hiring vendor?

Derek Mobley filed a class action (Case No. 3:23-cv-00770-RFL, N.D. Cal.) on February 21, 2023, alleging Workday's AI applicant screening system discriminates based on race, age (over 40), and disability — violating Title VII, the ADEA, the ADA, and California anti-discrimination law. Critically, Workday is not Mobley's employer; it is a third-party vendor. The case puts an AI vendor directly in the defendant seat under an "agent" liability theory.

Derek Mobley filed a class action complaint on February 21, 2023, in the U.S. District Court for the Northern District of California against Workday, Inc. — Case No. 3:23-cv-00770-RFL, assigned to Judge Rita F. Lin.

Mobley alleges that Workday's AI-powered applicant recommendation system — used by employers to score, sort, rank, and screen job applicants — unlawfully discriminates on the basis of race (African American), age (over 40), and disability. The claims arise under Title VII of the Civil Rights Act, the Age Discrimination in Employment Act (ADEA), the Americans with Disabilities Act (ADA), and California's anti-discrimination laws. Mobley applied to over 100 positions through employers using Workday's platform and was rejected without interviews each time. He alleges the AI tool's design reflected employer biases and relied on biased training data that produced systemic exclusions across protected categories.

What makes this case unusual is that Workday was not Mobley's employer or prospective employer. Workday is a third-party HR technology vendor whose software is used by thousands of other companies to make hiring decisions. The case puts the vendor — not just the employers — in the defendant seat.

## What are the key rulings in Mobley v. Workday that advanced the AI vendor liability theory?

Two rulings advanced the case: On July 12, 2024, the court denied Workday's motion to dismiss, allowing disparate impact claims to proceed on an agent liability theory while dismissing intentional discrimination and aiding-and-abetting claims. On May 16, 2025, the court granted conditional certification of the ADEA collective action, finding Workday's unified AI screening system sufficient to certify a class despite employer-clients' varied configurations of the tool.

The case has cleared two significant procedural hurdles.

On July 12, 2024, the court denied Workday's second motion to dismiss. The court rejected the argument that Workday qualifies as an "employment agency" under federal law — but it allowed Mobley's disparate impact discrimination claims to proceed on an "agent" theory of liability. Under this theory, Workday may be held directly liable as an agent of the employers who use its software to make hiring decisions. The court dismissed Mobley's intentional discrimination and aiding-and-abetting claims under California law but preserved the core federal disparate impact claims ([GovInfo court filing, May 16, 2025 order referencing prior rulings](https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf)).

On May 16, 2025, the court granted conditional certification of the ADEA collective action claims. The court found that Mobley sufficiently alleged a unified policy — Workday's AI applicant screening system — affecting all putative class members. Workday argued that different employer-clients' varied use of the tools and differing applicant qualifications made collective treatment inappropriate, but the court ruled those differences were immaterial for certification purposes ([GovInfo court order](https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf)).

An October 2, 2025, order directed the parties to prepare a dissemination plan for notifying potential opt-in plaintiffs. As of March 2026, the case remains active, with discovery ongoing and the last known filing dated March 27, 2026.

## Why does Workday's disclosure of 1.1 billion rejected applications matter for the Mobley lawsuit?

Workday disclosed in court filings that approximately 1.1 billion applications were rejected using its software tools from September 24, 2020, to the present — meaning the conditionally certified ADEA collective could number in the hundreds of millions of job seekers. This would make Mobley potentially one of the largest employment discrimination collectives ever certified in a federal court, substantially increasing the financial stakes for Workday and for employers whose AI screening tools face similar claims.

During the conditional certification proceedings, Workday disclosed in court filings that approximately 1.1 billion applications were rejected using its software tools during the relevant time period (from September 24, 2020, to the present). This means the potential ADEA collective could number in the "hundreds of millions" of job seekers — making it potentially one of the largest collectives ever conditionally certified in an employment discrimination case ([GovInfo court order, May 16, 2025](https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf)).

## What is the "agent" liability theory that allowed Mobley v. Workday to proceed against an AI vendor?

Title VII, the ADEA, and the ADA define "employer" to include "any agent" of the employer. Mobley argues that when employers delegate hiring screening to Workday's AI system, Workday acts as their agent and can be held directly liable for discriminatory outcomes. The court ruled this theory was plausible enough to survive dismissal — establishing that AI vendors who design and operate screening tools used in hiring decisions may face direct claims from affected applicants, not just contractual disputes with employer-clients.

The central legal innovation of this case is the "agent" theory of liability. Under Title VII, the ADEA, and the ADA, the definition of "employer" includes "any agent" of the employer. Mobley argues that when employers delegate hiring decisions to Workday's AI screening system, Workday acts as an agent of those employers and can therefore be held directly liable for discriminatory outcomes.

The court agreed this theory was at least plausible enough to survive dismissal. This matters because, until this ruling, it was generally assumed that AI vendors occupied a legal gray zone — they might bear contractual responsibility to their clients, but direct liability to the job applicants screened by their tools was an open question. If this theory is ultimately upheld, it would establish that AI vendors who design and operate screening tools used in hiring decisions can be sued directly by affected applicants under federal anti-discrimination law.

## What position did the EEOC take in its Mobley v. Workday amicus brief?

The EEOC filed a 21-page amicus brief on April 9, 2024, advancing three liability theories: Workday as an "employment agency" that screens and refers applicants; Workday as an "indirect employer" that controls applicants' access to employment opportunities; and Workday as an "agent" of employers who have delegated hiring authority to its AI system. The EEOC's prior guidance explicitly states that agents "may include entities such as software vendors" when employers grant them authority to act on their behalf.

The U.S. Equal Employment Opportunity Commission (EEOC) filed a 21-page [amicus curiae brief](https://www.eeoc.gov/litigation/briefs/mobley-v-workday-inc) on April 9, 2024, in support of plaintiff Mobley ([full brief PDF](https://www.eeoc.gov/sites/default/files/2024-04/Mobley%20v%20Workday%20NDCal%20am-brf%2004-24%20sjw.pdf)). The brief, signed by EEOC General Counsel Karla Gilbride, advanced three distinct theories of liability:

**Employment agency theory:** The EEOC argued that Workday plausibly operates as an "employment agency" because it engages to a significant degree in screening and referral activities traditionally associated with employment agencies (Brief at pp. 4–10).

**Indirect employer theory:** The EEOC argued Workday is an "indirect employer" because it exercises significant control over applicants' access to employment opportunities. The relevant inquiry, per the EEOC, is not whether Workday controls employers' day-to-day operations but whether it can control or interfere with applicants' *access* to those employers (Brief at pp. 10–12).

**Agent theory:** The EEOC argued Workday is an "agent" of employers because employers have delegated hiring decision authority to Workday's system. The brief cited case law establishing that where an employer delegates "functions traditionally exercised by an employer" to a third party, that third party can be found to be an employer by virtue of the agency relationship. The EEOC cited its own prior guidance stating that employer's agents "may include entities such as software vendors, if the employer has given them authority to act on the employer's behalf" (Brief at pp. 12–14).

The court ultimately allowed the agent theory to proceed while dismissing the employment agency theory. But the EEOC's endorsement of all three theories signals that the federal enforcement agency views AI vendor accountability as squarely within the reach of existing civil rights law.

## What does Mobley v. Workday mean for employers using any AI hiring tool?

Any employer using an AI screening tool faces the same exposure: if that system produces discriminatory outcomes, the employer faces claims as principal and the vendor may face them as agent. AI adoption in HR reached 43% in 2025, up from 26% in 2024 — the pool of potentially exposed employers is growing rapidly. Employers should conduct or require bias audits of AI hiring tools, document selection and monitoring processes, and review vendor contracts to understand who bears responsibility for discriminatory outcomes.

If you use any AI screening tool — not just Workday's — this case affects you. The agent liability theory does not stop at Workday. It applies to any vendor whose system performs a hiring function on your behalf. If that system produces discriminatory outcomes, you may face claims as the principal, and your vendor may face them too. The ADEA conditional certification here means the plaintiff class can number in the hundreds of millions. That is not a small-claims problem.

AI adoption in HR tasks climbed to 43% in 2025, up from 26% in 2024 — the pool of employers potentially facing this exposure is growing rapidly. Workday alone reports more than 10,000 customers worldwide. The companies using AI in hiring decisions are no longer edge cases; they are the majority.

Practically, employers should be conducting or requiring bias audits of their AI hiring tools, documenting their selection and monitoring processes, and reviewing vendor contracts to understand who bears responsibility for discriminatory outcomes. Our [AI Bias Audit Report Template](/products/ai-bias-audit-template) provides the framework for conducting these audits. The [EEOC AI Hiring Compliance Kit](/products/eeoc-ai-hiring) covers the full Title VII and UGESP documentation requirements. And the [Vendor AI Due Diligence Kit](/products/vendor-ai-due-diligence) gives you the questionnaire and contract addendum templates to hold vendors accountable.

## What does the Workday agent liability theory mean for AI vendors selling HR technology?

If upheld, the agent theory means AI vendors who build, operate, or maintain hiring screening tools can face direct lawsuits from rejected applicants — even without any direct employment relationship with those applicants. Vendors can no longer assume contractual indemnification clauses with employer-clients fully insulate them. They face pressure to proactively publish disparate impact analyses, provide audit trails for algorithmic decisions, and redesign business models to account for the possibility they are legally agents of every employer using their platform.

For AI vendors in the HR technology space, this case represents a fundamental shift in potential liability. If upheld, the agent theory means that vendors who build, operate, or maintain AI hiring tools can face direct lawsuits from the applicants those tools screen — even though the vendor has no direct employment relationship with those applicants.

This changes the vendor relationship in several ways. Vendors can no longer assume that contractual indemnification clauses with employer-clients fully insulate them from liability. They may need to proactively conduct and publish disparate impact analyses. They face pressure to provide audit trails and explainability for algorithmic decisions. And they may need to redesign their business models to account for the possibility that they are, legally speaking, agents of every employer that uses their platform.

## What other active AI employment discrimination cases are shaping vendor and employer liability?

The EEOC's 2023 settlement with iTutorGroup (EEOC v. iTutorGroup, No. 1:22-cv-02565, E.D.N.Y.) was the first resolved federal AI hiring discrimination case — iTutorGroup paid $365,000 after its software automatically rejected female applicants aged 55+ and male applicants aged 60+. In March 2025, the ACLU filed complaints against Intuit and HireVue for alleged AI-driven hiring discrimination. Together with Mobley, these cases form a growing litigation landscape testing whether existing civil rights law covers algorithmic bias in employment.

Mobley is not the only case in this space. The EEOC's settlement with iTutorGroup in September 2023 was the first federal AI hiring discrimination case to reach resolution. In [EEOC v. iTutorGroup, Inc., Civil Action No. 1:22-cv-02565](https://www.eeoc.gov/newsroom/itutorgroup-pay-365000-settle-eeoc-discriminatory-hiring-suit) (E.D.N.Y.), the EEOC alleged that iTutorGroup programmed its tutor application software to automatically reject female applicants aged 55 or older and male applicants aged 60 or older, rejecting more than 200 qualified applicants. The company paid $365,000 to settle the suit.

In March 2025, the ACLU filed complaints against Intuit and HireVue related to alleged AI-driven hiring discrimination. These complaints remain in earlier stages. Together with Mobley, these cases represent a growing body of litigation testing whether existing civil rights frameworks can adequately address algorithmic bias in employment.

## What did the NY State Comptroller's audit find about NYC Local Law 144 enforcement of AI hiring bias audits?

The December 2, 2025 Comptroller's audit (Report No. 2024-N-6) covering July 2023–June 2025 found that DCWP received only two AEDT complaints during the entire period and identified just one non-compliance instance among 32 companies reviewed. The Comptroller's own review of the same 32 companies found at least 17 instances of potential non-compliance — revealing that DCWP's complaint-based model is fundamentally ineffective, complaint routing through 311 was broken, and no stakeholder outreach had been conducted since the law took effect.

On December 2, 2025, the [New York State Comptroller released an audit](https://www.osc.ny.gov/state-agencies/audits/2025/12/02/enforcement-local-law-144-automated-employment-decision-tools) of NYC's enforcement of [Local Law 144 of 2021](https://www.nyc.gov/site/dca/about/automated-employment-decision-tools.page), the first municipal law in the United States requiring bias audits of automated employment decision tools (AEDTs). The audit (Report No. 2024-N-6) covered the period from July 2023 through June 2025.

The findings were stark. DCWP received only two AEDT-related complaints during the entire audit period. DCWP surveyed the websites and bias audits of 32 companies and found just a single instance of non-compliance. However, the Comptroller's office reviewed the same 32 companies and identified at least 17 instances of potential non-compliance — a dramatically higher number that suggests DCWP's review was not using the formal procedures created by the NYC Office of Technology and Innovation.

The audit concluded that DCWP's complaint-based enforcement model is fundamentally ineffective. The complaint intake process through the 311 system was not properly routing AEDT complaints to DCWP. No additional stakeholder education or outreach had been performed since the law took effect. And DCWP did not consult with OTI's technical experts when making determinations about AEDT use, despite lacking internal technical expertise.

*The EEOC's full 21-page amicus brief is publicly available on the [EEOC's website](https://www.eeoc.gov/sites/default/files/2024-04/Mobley%20v%20Workday%20NDCal%20am-brf%2004-24%20sjw.pdf). The three-theory analysis above is drawn directly from the brief text. The July 12, 2024 ruling description is sourced from the May 16, 2025 [GovInfo order](https://www.govinfo.gov/content/pkg/USCOURTS-cand-3_23-cv-00770/pdf/USCOURTS-cand-3_23-cv-00770-0.pdf) referencing prior rulings.*

